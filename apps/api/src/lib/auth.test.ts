...
describe("hasValidApiToken", () => {
  ...
  const load = async () => (await import("./auth.js")).hasValidApiToken;

  it("is false when API_TOKEN is not configured", async () => {
    delete process.env.API_TOKEN;
    vi.resetModules();
    const hasValidApiToken = await load();

    expect(hasValidApiToken(makeRequest("Bearer anything"))).toBe(false);
  });

  it("is false without a bearer header and for a wrong token", async () => {
    process.env.API_TOKEN = "secret-token";
    vi.resetModules();
    const hasValidApiToken = await load();

    expect(hasValidApiToken(makeRequest(undefined))).toBe(false);
    expect(hasValidApiToken(makeRequest("secret-token"))).toBe(false);
    expect(hasValidApiToken(makeRequest("Bearer wrong-token"))).toBe(false);
  });

  it("is true for the configured token", async () => {
    process.env.API_TOKEN = "secret-token";
    vi.resetModules();
    const hasValidApiToken = await load();

    expect(hasValidApiToken(makeRequest("Bearer secret-token"))).toBe(true);
  });
});
