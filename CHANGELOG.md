...
- Profile resolution is cached for 30s (and cleared whenever a profile is created, updated or deleted), so the ~11 requests the dashboard fires on mount share one profile lookup instead of each paying its own. The PIN gate is unaffected — it is re-evaluated per request against the cached profile.
